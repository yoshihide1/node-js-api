...
app.get('/api/v1/total/', (req, res) => {
  connection.query(`select date_format(created_at, '%Y-%m-%d') as date, sum(cases) as cases, sum(pcr) as pcr, sum(deaths) as deaths from corona group by date_format(created_at, '%Y%m%d') order by created_at desc limit 14`, (error, results , fields) => {
    if (error) {
      console.log(error)
    } else {
      res.json(results)
